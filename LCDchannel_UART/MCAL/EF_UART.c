...
BOOLEAN EF_BOOLEAN_UART_GetU16 ( U16_t* Data_ptr)
{
	U8_t ReturnedState = 0;
	U16_t TempData = 0;
	/* receive the MSByte First */
	ReturnedState = EF_BOOLEAN_UART_GetChar(  (U8_t*) &TempData );
	...
	else
	{
		*Data_ptr = TempData<<8;
		...
		{
			*Data_ptr = (*Data_ptr)| (TempData& 0xFF);
