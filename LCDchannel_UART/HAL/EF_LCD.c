...
void EF_void_LCD_init()
{
    ...
    UART_cfg_str  uart_cfg = {9600 , 8, ONE_STOP_BIT, NO_PARITY, TRUE, FALSE, TRUE, TRUE};

    EF_void_UART_Init( &uart_cfg );

	/* Init Control PIN as Output */
	LCD_CNTRL_DDR |= (1 << LCD_RS_PIN | 1 << LCD_ENABLE_PIN);
	LCD_CNTRL_PORT = 0x00;
//
...
/* using by ISR, for Send data or Command */
...
* Return Value: Void.
***********************************************************************/
void EF_void_LCD_print_NUM(S16_t s16Numberstring, U8_t u8RowNumber, U8_t u8colNumber)
{
	U8_t ArrayIndex = 0;
	...
	{
		EF_void_LCD_goto(u8RowNumber, (ArrayIndex + u8colNumber));
...
ISR(USART_RXC_vect)
{
	...
	if (RX_data == COMMAND_MODE )
	{
		LCD_CNTRL_PORT &= ~(1<<LCD_RS_PIN);

	}
	else if (RX_data == DATA_MODE  )
	{
		LCD_CNTRL_PORT &= ~(1<<LCD_RW_PIN);
		LCD_CNTRL_PORT |= (1<<LCD_RS_PIN);
	}
