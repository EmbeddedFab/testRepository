...
BOOLEAN EF_BOOLEAN_UART_GetChar(U8_t UART_Number,U8_t * returnedValue)
{
	 ...
	 * * */
    while( (!EF_BOOLEAN_TimerCheck(UART_TIMER_ID)) &&
            (!( UARTCharsAvail(UART_PinConfiguration_Array[UART_Number][UART_BASE_ELEMENT]))) );
...
BOOLEAN EF_BOOLEAN_UART_CheckForRxData(U8_t UART_Number)
{
    ...
    return UARTCharsAvail(UART_PinConfiguration_Array[UART_Number][UART_BASE_ELEMENT]);
}
/****************************************************************************
